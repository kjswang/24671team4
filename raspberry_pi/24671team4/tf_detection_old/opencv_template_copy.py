...
#     blur = cv2.GaussianBlur(image,(7,7),0)
    ...
    #for i in range(x):
     #   for j in range(y):
      #      img_gray[i][j] = (img_gray[i][j]/255)*100
    #print(sum)
    #(thresh, img_gray) = cv2.threshold(image, 100, 255, cv2.THRESH_BINARY)
    template = cv2.imread('Layer1_blackbg.png',0)
    #(thresh, template) = cv2.threshold(template, 100, 255, cv2.THRESH_BINARY)
    h,w = template.shape
    
    methods = [cv2.TM_CCOEFF, cv2.TM_CCOEFF_NORMED, cv2.TM_CCORR,cv2.TM_CCORR_NORMED,cv2.TM_SQDIFF,cv2.TM_SQDIFF_NORMED]
    
    method = methods[2]
    #d1=cv2.matchShapes(img_gray,template,cv2.CONTOURS_MATCH_I1,0)
    #d2=cv2.matchShapes(img_gray,template,cv2.CONTOURS_MATCH_I2,0)
    #d3=cv2.matchShapes(img_gray,template,cv2.CONTOURS_MATCH_I3,0)

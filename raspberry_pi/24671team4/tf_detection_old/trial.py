...
#     blur = cv2.GaussianBlur(image,(7,7),0)
    ...
    method = methods[2]
    dst, threshold = cv2.threshold(img_gray, 100, 255, cv2.THRESH_BINARY)
    contours,_ = cv2.findContours(threshold, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    for i in range(len(contours)):
        d=cv2.matchShapes(template,contours[i],cv2.CONTOURS_MATCH_I2,0)
        print(d)
        if d<2:
            img_gray = cv2.drawContours(img_gray, contours, i,(0,0,255),-1)
